...
$count="SELECT COUNT(*) FROM reflexioncierrepdf";
 ...
 mysqli_set_charset($conecta,"utf8");
  ...
  {
    if( $rows['idReflexionCPdf'] ) {
      
      $varCadena = strlen ($rows["anexoR"]);
      if($varCadena>0)
      {
        $enlaceHref = " <a href=../pdf/".$rows["anexoR"]."> ";
    ...
    echo "<td>".$rows['descripcionRefPdf']."</td>
          ...
          <td style='text-align:center'><a href=../pdf/".$rows["pdfRefC"]."><img src='../images/ver.png' width='40%'></a></td>
           <td> ".$enlaceHref." ". $no_pdf ."</a></td>
          <td> <a href='actualizarCierrePdf.php?id=".$rows['idReflexionCPdf']."'> <input  class='btn btn-success' style='color: white; font-size: 14px' type='button' value=' Actualizar'></a></td>";


    echo "</tr>";
    }
    if(['idReflexionCPdf'] < $id ) {
      echo "<tr>";
      echo "<td>".$rows['TituloRefPdf']."</td>";
      echo "<td>".$rows['descripcionRefPdf']."</td>
