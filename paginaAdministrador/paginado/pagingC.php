...
$count="SELECT COUNT(*) FROM tda where idTipoTdas='2'";
 ...
 mysqli_set_charset($conecta,"utf8");
  ...
  echo "<th class='t5' style='background:#29a900; color:white;width:5%; text-align:center'><b>Anexo</b></th>";
  echo "<th class='t6' style='background:#29a900; color:white; text-align:center'><b>Actualizar</b></th>";
  echo "<th class='t6' style='background:#29a900; color:white;text-align:center'><b>Eliminar</b></th>";
  echo "</tr>";
  echo "</thead>";
  echo "<tbody>";
  ...
  while($rows = $c->fetch_assoc())
  {
    if( $rows['idTda']) {
      ...
      echo "<td>".$rows['descripcionTda']."</td>
            <td><a href=../pdf/".$rows["rutaPDF"]."><img src='../images/ver.png' width='50%' text-align:center></a></td>
            
            <td> ".$enlaceHref." ". $no_pdf ."</a></td>";
      echo "<td> <a href='actualizarContex.php?id=".$rows['idTda']."' > <input  class='btn btn-success' style='color: white; font-size: 14px' type='button' value=' Actualizar'></a></td>";
      
      echo "<td id='t9'> <a href='eliminarContex.php?id=".$rows['idTda']."' onclick=\"return confirmar('¿Está seguro que desea eliminar el registro?')\"><input  class='btn btn-danger' style='color: white; font-size: 14px' type='button' value=' Eliminar'></a></td>";
